...
############## BBS Core ###################
my $BBS_OBJ = BBS::Universal->small_new({ 'debug' => $DEBUG, 'debuglevel' => $LEVEL });
...
my $COMMANDS = {
    ...
    'STATISTICS' => sub {
        my $bbs_obj = shift;
        $bbs_obj->sysop_parse_menu(($START_ROW + $ROW_ADJUST + 1), 'files/main/statistics.txt');
    },
    'BACK' => sub {
    },
    'EDIT USERS' => sub {
...
sub logo {
    ...
    print setscroll(1, $hsize), locate(1, 1), cldown, colored(['on_white'], clline), "\n";
    print color('red');
    print '  ____  ____ ____    _   _       _                          _  ' . "\n";
    print color('yellow');
    print ' | __ )| __ ) ___|  | | | |_ __ (_)_   _____ _ __ ___  __ _| |' . "\n";
    print color('green');
    print q{ |  _ \|  _ \___ \  | | | | '_ \| \ \ / / _ \ '__/ __|/ _` | |} . "\n";
    print color('magenta');
    print ' | |_) | |_) |__) | | |_| | | | | |\ V /  __/ |  \__ \ (_| | |' . "\n";
    print color('bright_blue');
    print ' |____/|____/____/   \___/|_| |_|_| \_/ \___|_|  |___/\__,_|_|' . "\n";
    print color('reset');
    print locate(3, 65),         sprintf('Version %.03f', $BBS::Universal::VERSION), "\n";
    print locate(4, 65),         'Written By Richard Kelsch',                       clline;
    print locate(4, 65),         'Copyright © 2023 Richard Kelsch',                 clline;
    print locate(5, 65),         'All Rights Reserved',                             clline;
    print locate(6, 65),         'Licensed under the GNU Public License Version 3', clline;
...
sub system_menu {
    ...
    print locate($START_ROW, 1), cldown, 'Loading ' . $MAX_THREADS . ' Threads ...';
    ...
    while ($RUNNING) {
        ...
        my $command = $bbs_obj->sysop_parse_menu(($START_ROW + $ROW_ADJUST + 1), 'files/main/sysop.txt');

        $COMMANDS->{$command}->($bbs_obj);
        threads->yield();
    } ## end while ($RUNNING)
...
sub clean_joinable {
    ...
    foreach my $thread (sort(keys %{$SERVER_THREADS})) {
        ...
        servers_status($bbs_obj, FALSE);
        alarm(0);
    } ## end foreach my $thread (sort(keys...))
