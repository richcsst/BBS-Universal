...
sub parse_telnet_escape {
	...
	if ($command == WILL) {
		if ($option == ECHO) { # WON'T ECHO
			$self->{'debug'}->DEBUG(['Received IAC WILL ECHO']);
			print $handle chr(IAC) . chr(WONT) . chr(ECHO);
			$self->{'debug'}->DEBUG(['Sent IAC WONT ECHO']);
		} elsif ($option == LINEMODE) {
			$self->{'debug'}->DEBUG(['Received IAC WILL LINEMODE']);
			print $handle chr(IAC) . chr(WONT) . chr(LINEMODE);
			$self->{'debug'}->DEBUG(['Sent IAC WONT LINEMODE']);
		}
	} elsif ($command == DO) {
		if ($option == ECHO) { # DON'T ECHO
			$self->{'debug'}->DEBUG(['Received IAC DO ECHO']);
			print $handle chr(IAC) . chr(DONT) . chr(ECHO);
			$self->{'debug'}->DEBUG(['Sent IAC DONT ECHO']);
		} elsif ($option == LINEMODE) {
			$self->{'debug'}->DEBUG(['Received IAC DO LINEMODE']);
			print $handle chr(IAC) . chr(DONT) . chr(LINEMODE);
			$self->{'debug'}->DEBUG(['Sent IAC DONT LINEMODE']);
		}
	} else {
    ...
    if ($self->{'local_mode'}) {
		...
		do {
			...
			if ($key eq chr(255)) { # IAC sequence
				my $command = ReadKey($self->{'USER'}->{'timeout'} * 60, $handle);
				my $option  = ReadKey($self->{'USER'}->{'timeout'} * 60, $handle);
