...
BEGIN {
    ...
    our $ANSI_VERSION = '0.003';
    our $ASCII_VERSION = '0.002';
    our $ATASCII_VERSION = '0.003';
    ...
    our $PETSCII_VERSION = '0.003';
    our $SYSOP_VERSION = '0.004';
...
sub populate_common {
	...
	$self->{'debug'}->INFO(['Initializing all libraries']);
    $self->db_initialize();
	...
	$self->{'debug'}->INFO(['Libraries initialized']);
    chomp(my $os = `uname -a`);
    ...
    $self->{'TOKENS'} = {
        ...
        'UPTIME' => sub {
			...
			$self->{'debug'}->INFO(["Get Uptime $uptime"]);
            return ($uptime);
        },
        'SHOW BBS LIST' => sub {
            my $self = shift;
	...
	$self->{'debug'}->INFO(['Main object initialized']);
}
...
sub login {
    ...
    my $username;
    if ($self->{'sysop'}) {
		$self->{'debug'}->INFO(['Login as SysOp']);
        $username = 'sysop';
...
sub create_account {
	...
	$self->{'debug'}->INFO(['Create account']);
    return (FALSE);
}
...
sub decision {
    ...
    if ($response eq 'Y') {
        $self->output("YES\n");
		$self->{'debug'}->INFO(['Decision YES']);
        return (TRUE);
    }
	$self->{'debug'}->INFO(['Decision NO']);
    $self->output("NO\n");
    return (FALSE);
}
...
sub load_menu {
	...
	$self->{'debug'}->INFO(["Load Menu $file"]);
...
sub parse_telnet_escape {
	...
	$self->{'debug'}->INFO(['Parse Telnet Escape']);
	if ($command == WILL) {
...
sub detokenize_text {    # Detokenize text markup
	...
	$self->{'debug'}->INFO(['Detokenize Text']);
...
sub static_configuration {
	...
	$self->{'debug'}->INFO(['Static Configuration']);
...
sub choose_file_category {
	...
	$self->{'debug'}->INFO(['Choose File Category']);
    my $table;
...
sub parse_versions {
	...
	$self->{'debug'}->INFO(['Parse Versions']);
###
    my $versions = {
...
sub playit {
	...
	unless($self->{'nosound'}) {
		$self->{'debug'}->INFO(["Play Sound $file"]);
