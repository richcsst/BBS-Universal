...
sub new {    # Always call with the socket as a parameter
    ...
    my $self = {
		...
		'telnet_commands' => [
			'SE (Subnegotiation end) - End of negotiation (or data block) of a sub-service of a protocol mechanism',
			'NOP (No operation) - Data packet that does nothing',
			'Data Mark',
			'Break',
			'Interrupt Process - Request that other party ends current process',
			'Abort output - Request that other party stops sending output',
			'Are you there?',
			'Erase character',
			'Erase Line',
			'Go ahead',
			'SB (Subnegotiation begin) - Initiate the negotiation of a sub-service of a protocol mechanism',
			'WILL - Informs othqer party that this party will use a protocol mechanism',
			"WON'T - Informs other party that this party will not use a protocol mechanism",
			'DO - Instruct other party to use a protocol mechanism',
			"DON'T - Instruct other party to not use a protocol mechanism",
			'IAC - Sequence Initializer/Escape Character',
		],
		'telnet_options'  => [
			'Binary Transmission',
			'Echo',
...
sub populate_common {
    ...
    $self->{'COMMANDS'} = {
		...
		'FORUM CATEGORIES' => sub {
            ...
            return ($self->load_menu('files/main/forum'));
		},
...
sub prompt {
    ...
    if ($self->{'USER'}->{'text_mode'} eq 'ATASCII') {
        ...
        $response = $text . ' ' . $self->{'ansi_characters'}->{'BLACK RIGHT-POINTING TRIANGLE'} . ' ';
    } else {
        $response = "$text > ";
    }
    return ($response);
...
sub menu_choice {
    ...
    if ($self->{'USER'}->{'text_mode'} eq 'ATASCII') {
        ...
        $self->output($self->{'ansi_characters'}->{'BOX DRAWINGS LIGHT VERTICAL'} . colored([$color], $choice) . $self->{'ansi_characters'}->{'BOX DRAWINGS LIGHT VERTICAL'} . colored([$color], $self->{'ansi_characters'}->{'BLACK RIGHT-POINTING TRIANGLE'}) . " $desc");
    } else {
        $self->output(" $choice > $desc");
    }
} ## end sub menu_choice
...
sub parse_telnet_escape {
	...
	return(sprintf('%s - %s',$self->{'telnet_commands'}->[($command - 240)],$self->{'telnet_options'}->[$option]));
}
...
sub get_key {
    ...
    if ($self->{'local_mode'}) {
		...
		my $escape = FALSE;
		ReadMode 'noecho', $self->{'cl_socket'};
		do {
			$key = ($blocking) ? ReadKey($self->{'USER'}->{'timeout'} * 60, $self->{'cl_socket'}) : ReadKey(-1, $self->{'cl_socket'});
			if ($key eq chr(255)) { # Ignore Telnet escapes
				$self->{'debug'}->DEBUG(['IAC Telnet Escape Sequence']);
				$escape = TRUE;
				my $command = ReadKey($self->{'USER'}->{'timeout'} * 60, $self->{'cl_socket'});
				my $option  = ReadKey($self->{'USER'}->{'timeout'} * 60, $self->{'cl_socket'});
				$self->{'debug'}->DEBUG([$self->parse_telnet_escape(ord($command),ord($option))]);
			} else {
				$escape = FALSE;
			}
    ...
    $self->{'debug'}->DEBUGMAX(["Key pressed - $key - " . ord($key)."  ECHO=$echo  BLOCKING=$blocking"]);
